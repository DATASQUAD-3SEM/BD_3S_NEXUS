# Regras do Grupo - Critérios de permanencia
**Efetivo desde 07/09/2026**

---

## 1. Atualização no JIRA
(*Ao menos 3x por semana*)

Deve ser feito um aviso no **"Quadro" do JIRA** informando o progresso nas seguintes categorias:

- **A fazer:** O que ainda precisa ser feito.  
- **Em andamento:** O que aquele indivíduo está fazendo no momento.
- **Em analise:** Faze de teste. 
- **Concluído:** O que foi finalizado.  
- **Dificuldades:** *(Opcional)* Apenas caso exista uma razão para não ter conseguido realizar o requisito.

> Em caso de **Dificuldades**, todos os integrantes (exceto quem postou) decidirão se a justificativa é válida.

---

## 2. Aprendizado de GitHub
Um integrante de **cada time** deve aprender a realizar alterações no GitHub, incluindo:

- **Commits**
- **Pull Requests**
- **Merges**

Para isso, é necessário ler o arquivo:

CONTRIBUTING.md

Localizado na **página inicial do nosso repositório no GitHub**.

---

## 3. Atenção ao Grupo
Todo integrante deve ficar atento às mensagens do **grupo** e às tarefas no **JIRA**.

Exemplo:
- Se uma tarefa **X** foi publicada no grupo ou no JIRA, todos os envolvidos devem responder ou agir.

Não haverá mais insistência para respostas.  
A ausência de resposta poderá resultar em **redução nos pontos PACER**.

---

## 4. Pontuação PACER
> **Aqueles que ao final de uma sprint não cumprirem ao menos 2 pontos PACER em cada requisito serão removidos do grupo.**

---

## 5. Alteração de Regras
Quaisquer regras acima podem ser **alteradas mediante votação da maioria do grupo**.
